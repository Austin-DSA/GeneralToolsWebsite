...
class AccessToken:
    ...
    def __init__(
        self, token: str, tokenType: str, expiresInSec: int, scope: str, apiUrl: str
    ) -> None:
        self.token = token
        self.tokenType = tokenType
        self.scope = scope
        self.apiUrl = apiUrl
        self.expireTime = datetime.datetime.now() + datetime.timedelta(
...
class ZoomAPI:
    ...
    def _refreshAccessToken(self) -> None:
        ...
        authSecretsBase64 = base64.standard_b64encode(
            f"{self._clientId}:{self._clientSecret}".encode()
        )
        ...
        req = requests.post(
            url=Constants.AccessToken.OAUTH_ENDPOINT, data=body, auth=auth
        )
        ...
        self._accessToken = AccessToken(
            ...
            expiresInSec=int(
                responseDict[Constants.AccessToken.RESPONSE_EXPIRES_IN]
            ),  # This should be an int, but in case it is a string convert, should be cheap if already an int
            scope=responseDict[Constants.AccessToken.RESPONSE_SCOPE],
            apiUrl=responseDict[Constants.AccessToken.RESPONSE_API_URL],
        )
        logger.info(
            "ZoomAPI: Refreshed Access Token. Will expire at %s",
            str(self._accessToken.expireTime),
        )
    ...
    def _isAccessTokenValid(self) -> None:
        return self._accessToken is not None and datetime.datetime.now() < (
            self._accessToken.expireTime - datetime.timedelta(minutes=2)
        )
    ...
    def _fetchAccounts(self) -> list[ZoomUser]:
        def processJsonResponseIntoUsers(responseDict: dict) -> list[ZoomUser]:
            if Constants.Users.RESPONSE_USERS_KEY not in responseDict:
                logger.error(
                    "ZoomAPI: No users list in response, returning empty account list"
                )
                return []
            users = []
            for user in responseDict[Constants.Users.RESPONSE_USERS_KEY]:
                newUser = ZoomUser(
                    email=user[Constants.Users.RESPONSE_EMAIL_KEY],
                ...
                if newUser.status != Constants.Users.RESPONSE_USER_STATUS_ACTIVE:
                    logger.info(
                        "ZoomAPI: User %s is not active, instead %s. Skipping.",
                        newUser.email,
                        newUser.status,
                    )
                    continue
                ...
                req = requests.get(
                    Constants.Users.Features.SETTINGS_ENDPOINT(newUser.id),
                    headers=self._headersForRequest(),
                )
                req.raise_for_status()
                responseDict = req.json()
                newUser.features = ZoomUser.Features(
                    meetingCapacity=responseDict[
                        Constants.Users.Features.RESPONSE_FEATURE_KEY
                    ][Constants.Users.Features.RESPONSE_MEETING_CAPACITY]
                )
        ...
        accounts = []
        req = requests.get(
            Constants.Users.LIST_USERS_ENDPOINT, headers=self._headersForRequest()
        )
        req.raise_for_status()
        responseDict = req.json()
        ...
        while (
            Constants.NEXT_PAGE_TOKEN_KEY in responseDict
            and responseDict[Constants.NEXT_PAGE_TOKEN_KEY] != ""
        ):
            logger.info("ZoomAPI: Moving onto next page of account list")
            params = {
                Constants.NEXT_PAGE_TOKEN_KEY: responseDict[
                    Constants.NEXT_PAGE_TOKEN_KEY
                ]
            }
            req = requests.get(
                Constants.Users.LIST_USERS_ENDPOINT,
                headers=self._headersForRequest(),
                params=params,
            )
            req.raise_for_status()
    ...
    def _fetchMeetingsForAccountAndTime(
        ...
        def processJsonResponIntoMeetings(responseDict: dict) -> list[ZoomMeeting]:
            if Constants.Meetings.RESPONSE_MEETING_LIST_KEY not in responseDict:
                logger.error(
                    "ZoomAPI: No meeting list in response, returning empty meeting list"
                )
                return []
            meetings = []
            for meeting in responseDict[Constants.Meetings.RESPONSE_MEETING_LIST_KEY]:
                ...
                if (
                    meetingType == Constants.Meetings.TYPE_INSTANT
                    or meetingType == Constants.Meetings.TYPE_RECURRING_NO_FIXED
                ):
                    logger.info(
                        "ZoomAPI: Found meeting %s that is type %d, has no scheduled time so skipping",
                        meeting[Constants.Meetings.RESPONSE_TOPIC_KEY],
                        meetingType,
                    )
                    continue
                ...
                logger.info(
                    "ZoomAPI: Found meeting %s at %s, getting features",
                    newMeeting.topic,
                    startTime,
                )
        ...
        logger.info(
            "ZoomAPI: Fetching meetings for %s from %s to %s",
            account.email,
            fromDate,
            toDate,
        )
        ...
        req = requests.get(
            Constants.Meetings.MEETING_ENDPOINT(account.id),
            headers=self._headersForRequest(),
            params=params,
        )
        req.raise_for_status()
        ...
        while (
            ...
            params[Constants.NEXT_PAGE_TOKEN_KEY] = responseDict[
                Constants.NEXT_PAGE_TOKEN_KEY
            ]
            req = requests.get(
                Constants.Meetings.MEETING_ENDPOINT(account.id),
    ...
    def getAccountsAndAvailablilityForTime(
        ...
        logger.info(
            "ZoomAPI: Check availability for meeting starting at %s for duration %s",
            time,
            duration,
        )
        ...
        for account in self._accounts():
            ...
            potentialConflicts = self._fetchMeetingsForAccountAndTime(
                account=account, fromDate=fromDate, toDate=toDate
            )
            confirmedConflicts = []
            # Check for conflicts
            ...
            for potentialConflict in potentialConflicts:
                ...
                logger.info(
                    "ZoomAPI: Found conflict by meeting %s at %s for %s duration",
                    potentialConflict.topic,
                    potentialConflict.startTime,
                    potentialConflict.duration,
                )
    ...
    def createMeeting(
        ...
        req = requests.post(
            Constants.Meetings.MEETING_ENDPOINT(user.id),
            headers=self._headersForRequest(),
            json={
